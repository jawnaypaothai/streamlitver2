...
st.markdown("### กรอกตำแหน่งและประเภทจุดรองรับ")

colA1, colA2 = st.columns(2)
with colA1:
    support_pos_A = st.number_input("ตำแหน่งจุดรองรับ A (m)", min_value=0.0, max_value=L, value=0.0, key="suppA_pos")
with colA2:
    support_A = st.selectbox("ประเภทจุดรองรับ A", ["pinned", "roller", "fixed", "none"], key="suppA")

colB1, colB2 = st.columns(2)
with colB1:
    support_pos_B = st.number_input("ตำแหน่งจุดรองรับ B (m)", min_value=0.0, max_value=L, value=L, key="suppB_pos")
with colB2:
    support_B = st.selectbox("ประเภทจุดรองรับ B", ["roller", "pinned", "fixed", "none"], key="suppB")

if support_A != "none" and support_B != "none" and support_pos_A == support_pos_B:
    st.warning("❗ จุดรองรับ A และ B อยู่ตำแหน่งเดียวกัน อาจทำให้การวิเคราะห์ผิดพลาด")
